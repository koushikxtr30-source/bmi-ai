...
export function generateHealthPDF(
  ...
  if (aiPlan) {
    ...
    aiPlan.exerciseRoutine.forEach(day => {
      ...
      const label = day.rest ? `${day.day} - Rest Day` : `${day.day} - ${day.type} (${day.intensity}, ${day.duration})`
      doc.text(clean(label), col, y); nl(5)
      day.exercises.forEach(ex => bullet(ex)); nl(2)
    })

    checkPage(20); divider(); h2('Goal Timeline & Milestones')
    body(`Current BMI: ${aiPlan.timeline.currentBMI}  ->  Target BMI: ${aiPlan.timeline.targetBMI}  |  Target Weight: ${aiPlan.timeline.targetWeight} ${wUnit}`)
    nl(2)
    aiPlan.timeline.milestones.forEach(m => {
      checkPage(10)
      doc.setFontSize(8.5); doc.setFont('helvetica', 'bold'); doc.setTextColor(30,30,30)
      doc.text(clean(`Week ${m.week}  ${m.label}`), col, y); nl(4.5)
      body(`${m.target}`, 4); body(`Action: ${m.action}`, 4); nl(1)
    })
