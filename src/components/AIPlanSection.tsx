...
import {
  ...
  ChevronDown, Clock, CheckCircle2, Apple, Leaf,
} from 'lucide-react'
...
export function AIPlanSection({ plan, unitSystem }: AIPlanSectionProps) {
      ...
      <div className="rounded-xl border p-4" style={{ borderColor: goalColor + '44', background: goalBg }}>
        ...
        {plan.timeline.weeksToGoal > 0 && (
          <div className="flex items-center gap-4 mt-3 pt-3 border-t border-border/40">
            {[
              { val: plan.timeline.weeksToGoal, label: 'weeks' },
              { val: plan.timeline.weeklyChange, label: `${wUnit}/week` },
              { val: plan.timeline.targetWeight, label: `target ${wUnit}` },
            ].map(item => (
              <div key={item.label} className="text-center">
                <p className="text-lg font-bold" style={{ color: goalColor }}>{item.val}</p>
      ...
      <div className="rounded-xl border border-border overflow-hidden">
        ...
        {openSection === 'diet' && (
          <div className="px-4 pb-4 space-y-3">
            ...
            {(() => {
              const d = plan.dietOptions[dietTab]
              return (
                <div className="space-y-3">
                  ...
                  <div className="grid grid-cols-4 gap-2">
                    ...
                    {[
                      { label: 'Protein', val: `${d.macros.protein}g`, cls: 'bg-blue-500/10', color: 'text-blue-400' },
                      { label: 'Carbs',   val: `${d.macros.carbs}g`,   cls: 'bg-yellow-500/10', color: 'text-yellow-400' },
                      { label: 'Fat',     val: `${d.macros.fat}g`,     cls: 'bg-purple-500/10', color: 'text-purple-400' },
                    ].map(m => (
                      <div key={m.label} className={`${m.cls} rounded-lg p-2 text-center`}>
